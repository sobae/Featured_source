...
#if 1//ithong_hmpl
#define MC_DCT_SCRIPT_HMPL_IDENTIFIER    "hmp"
#define MC_DCT_SCRIPT_HMPL_ENTITY_PREFIX 'l'
#define MC_DCT_SCRIPT_HMPL_REF_LEN MC_DCT_SCRIPT_IDENTIFIER_LEN+1+2+2+4+4+2 //hmplscript_head
#define MC_DCT_SCRIPT_HMPL_BUFF_MAX 512

uint16_t hmpl_version = 1;
#endif
...
#if 1//ithong_hmpl
int mc_Script::SetHmplRaw(uint16_t mode, uint32_t svc_h, uint32_t svc_l, uint16_t size_info, unsigned char *info)
{
    unsigned char buf[MC_DCT_SCRIPT_HMPL_REF_LEN+MC_DCT_SCRIPT_HMPL_BUFF_MAX]; 
    int err;
    uint16_t step_size = 0;

    if((size_info>0) && ((info==NULL) || (size_info>MC_DCT_SCRIPT_HMPL_BUFF_MAX)))
    {
        return MC_ERR_INVALID_PARAMETER_VALUE;
    }
    ...
    memcpy(buf,MC_DCT_SCRIPT_HMPL_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    step_size += MC_DCT_SCRIPT_IDENTIFIER_LEN;
    buf[step_size++]=MC_DCT_SCRIPT_HMPL_ENTITY_PREFIX;

    mc_PutLE(&buf[step_size], &hmpl_version, 2); 
    step_size += 2;

    mc_PutLE(&buf[step_size], &mode, 2);
    step_size += 2;
	
    mc_PutLE(&buf[step_size], &svc_h, 4);
    step_size += 4;

    mc_PutLE(&buf[step_size], &svc_l, 4);
    step_size += 4;

    mc_PutLE(&buf[step_size], &size_info, 2);
    step_size += 2;

    mc_PutLE(&buf[step_size], info, size_info);
    step_size += size_info;

    return SetData(buf,step_size); 
}

int  mc_Script::GethmplRaw(uint16_t *mode, uint32_t *svc_h, uint32_t *svc_l, uint16_t *size_info, unsigned char *info)
{
    uint16_t local_ver = 0;
    uint16_t step_info = 0;
    ...
    check_size = m_lpCoord[m_CurrentElement*2+1];

    if(check_size < MC_DCT_SCRIPT_HMPL_REF_LEN)
    {
        return MC_ERR_INVALID_PARAMETER_VALUE;
    }
    ...
    if(memcmp(ptr, MC_DCT_SCRIPT_HMPL_IDENTIFIER, MC_DCT_SCRIPT_IDENTIFIER_LEN) != 0)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
    step_info += MC_DCT_SCRIPT_IDENTIFIER_LEN;
	
    if(ptr[step_info] != MC_DCT_SCRIPT_HMPL_ENTITY_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
    step_info ++;
    
    local_ver = (uint32_t)mc_GetLE(&ptr[step_info], 2);
    if(local_ver < hmpl_version)
    {
        return MC_ERR_INVALID_PARAMETER_VALUE;
    }
    step_info += 2;
    ...
    if(mode)
    {
        *mode = (uint16_t)mc_GetLE(&ptr[step_info], 2);
    }
    step_info += 2;

    if(svc_h)
    {
        *svc_h = (uint16_t)mc_GetLE(&ptr[step_info], 4);
    }
    step_info += 4;

    if(svc_l)
    {
        *svc_l = (uint16_t)mc_GetLE(&ptr[step_info], 4);
    }
    step_info += 4;
	
    real_size = (uint16_t)mc_GetLE(&ptr[step_info], 2);
    if(check_size != (real_size + MC_DCT_SCRIPT_HMPL_REF_LEN))
    {
        return MC_ERR_WRONG_SCRIPT;
    }
    ...
    if(info)
    {
        memcpy(info, &ptr[step_info], real_size);
    }
...
int  mc_Script::Gethmplinfo(hmplscript_head *head, unsigned char *info)
{
    uint16_t local_ver = 0;
    uint16_t step_info = 0;
    uint16_t check_size = 0;
    ...
    if(head)
    {
        memcpy((unsigned char *)head, &ptr[step_info], sizeof(hmplscript_head));
    }	
    step_info += sizeof(hmplscript_head);

    if((head->size > 0) && (head->size <= MC_DCT_SCRIPT_HMPL_BUFF_MAX) && (info!=NULL))
    {
        memcpy(info, &ptr[step_info], head->size);
    }
    
    return MC_ERR_NOERROR;
}

bool mc_Script::IsHmpl(void)
{
    uint16_t local_ver = 0;
    uint16_t step_info = 0;
    uint16_t check_size = 0;
...
#if 0//ithong_hmpl
int mc_Script::SetHmplRaw(hmplscript_head *head, unsigned char *info)
{
    unsigned char buf[MC_DCT_SCRIPT_HMPL_REF_LEN+MC_DCT_SCRIPT_HMPL_BUFF_MAX]; 
    int err;
    uint16_t step_size = 0;
    ...
    if((head->size > 0) && ((info==NULL) || (head->size > MC_DCT_SCRIPT_HMPL_BUFF_MAX)))
    {
        return MC_ERR_INVALID_PARAMETER_VALUE;
    }
    ...
    head->version = hmpl_version;
    mc_PutLE(&buf[step_size], (unsigned char *)head, sizeof(hmplscript_head)); 
    step_size += sizeof(hmplscript_head);
	
    mc_PutLE(&buf[step_size], info, head->size);
    step_size += head->size;
...
#if 0
int  mc_Script::GethmplMsg(HmplMsgType *msg)
{
    uint16_t local_ver = 0;
    uint16_t step_info = 0;
    uint16_t check_size = 0;
    ...
    memcpy((unsigned char *)&msg->head, &ptr[step_info], sizeof(hmplscript_head));

    if((msg->head.size > 0) && (msg->head.size <= MC_DCT_SCRIPT_HMPL_BUFF_MAX))
    {
        for(int m=0;m<msg->head.size;m++) msg->dataData.push_back(ptr[step_info+m])
    }
...
int mc_Script::ProcessHmpl(unsigned char *hmpl)
{
    if(hmpl==NULL)
    {
        return MC_ERR_WRONG_SCRIPT;
