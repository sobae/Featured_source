...
bool fImportAddrs = false;
//set<CTxDestination> candidateAddress;
extern CWallet* pwalletMain;
extern mc_WalletTxs* pwalletTxsMain;

bool addrImport(const CTxDestination &dest)
{
	string strLabel = "";
    ...
    pwalletMain->AddWatchOnly(GetScriptForDestination(dest));
...
bool mc_ExtractInputAssetQuantities(const CScript& script1, uint256 hash, string& reason)
{
    ...
    if (ithong) LogPrintf("[ASSET] mc_ExtractInputAssetQuantities: GetNumElements=%d \n", mc_gState->m_TmpScript->GetNumElements());
    ...
    for (int e = 0; e < mc_gState->m_TmpScript->GetNumElements(); e++)
    {
        ...
        if (ithong) LogPrintf("[ASSET] mc_ExtractInputAssetQuantities: GetAssetQuantities: err=%d \n", err);		
		...
		if (ithong) LogPrintf("[ASSET] mc_ExtractInputAssetQuantities: GetAssetGenesis: err=%d, quantity=%d \n", err,quantity);	
        if(err == 0)
        {
            ...
            if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&hash))
            {
                ...
                if(row>=0)
                {
                    ...
                    quantity+=last;
			if (ithong) LogPrintf("[ASSET] mc_ExtractInputAssetQuantities: mc_SetABQuantity: quantity=%d, last=%d \n", quantity,last);	
...
if (ithong) LogPrintf("[ASSET] mc_CompareAssetQuantities: m_TmpAssetsIn->GetCount()=%d :   \n",mc_gState->m_TmpAssetsIn->GetCount() );
	...
	if (ithong) LogPrintf("[ASSET] mc_CompareAssetQuantities:quantity=%d :   \n",quantity);
...
{    
    ...
    for (unsigned int j = 0; j < tx.vout.size(); j++)
    {
        ...
        {
        if (ithong) LogPrintf("[ASSET] IsOpReturnScript: GetNumElements=%d  \n", mc_gState->m_TmpScript->GetNumElements() );
			...
			if (ithong) LogPrintf("[ASSET] AcceptAssetGenesisFromPredefinedIssuers: new_entity_type=%d, details_script_type=%d, err=%d  \n",new_entity_type,details_script_type,err);
        ...
        {
        if (ithong) LogPrintf("[ASSET] none sOpReturnScript: GetNumElements=%d  \n", mc_gState->m_TmpScript->GetNumElements() );
		
            for (int e = 0; e < mc_gState->m_TmpScript->GetNumElements(); e++)
            {
                mc_gState->m_TmpScript->SetElement(e);
                err=mc_gState->m_TmpScript->GetAssetGenesis(&quantity);
			if (ithong) LogPrintf("[ASSET] none GetAssetGenesis: err=%d, quantity=%d, total=%d  \n", err, quantity, total );
                if(err == 0)
                {
                    new_issue=true;
                    if(quantity+total<0)
			...
			if (ithong) LogPrintf("[ASSET] GetAssetQuantities: err=%d   \n", err );
                    if((err != MC_ERR_NOERROR) && (err != MC_ERR_WRONG_SCRIPT))
                    {
	...
	if (ithong) LogPrintf("[ASSET] GetAssetQuantities: new_issue=%d , follow_on=%d, m_TmpAssetsOut->GetCount=%d  \n", new_issue, follow_on, mc_gState->m_TmpAssetsOut->GetCount() );
    ...
    *fFullReplayCheckRequired=true;
	...
	if (ithong) LogPrintf("[ASSET] GetAssetQuantities: new_issue=%d , issuers.size=%d  \n", new_issue,  issuers.size() );
                                 ...
                                 uint32_t *replay)
{
    ...
    bool fFullReplayCheckRequired;
    #ifdef FEATURE_HPAY_SUPPORT_BANNED_ADDRS
    bool fCheckBannedScript;
    #endif/* FEATURE_HPAY_SUPPORT_BANNED_ADDRS */
    ...
    #ifdef FEATURE_HPAY_SUPPORT_BANNED_ADDRS
    fCheckBannedScript = false;
    #endif/* FEATURE_HPAY_SUPPORT_BANNED_ADDRS */
     
    mc_gState->m_TmpAssetsIn->Clear();

   if (ithong) LogPrintf("[ASSET]AcceptHdacTransaction: tx=%s, tx.vin.size()=%d, tx.vout.size()=%d \n", tx.GetHash().ToString(), tx.vin.size(), tx.vout.size());
   //LogPrintf("AcceptHdacTransaction:   \n");
        ...
        for (unsigned int j = 0; j < tx.vout.size(); j++)
        {
            ...
            if(mc_gState->m_TmpScript->IsOpReturnScript())                      // OP_RETURN
            {
                ...
                if( (pass == 0) && (mc_gState->m_TmpScript->GetNumElements() == 2) ) // One OP_DROP + OP_RETURN - new entity or cached script
                {
                    ...
                    err=mc_gState->m_TmpScript->GetNewEntityType(&new_entity_type,&entity_update,details_script,&details_script_size);
			if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: 1096 : new_entity_type=%d, err=%d \n", new_entity_type ,err);
                    if(err == 0)    // New entity element
                    {
                        fScriptParsed=true;
			...
			if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: 1185 : GetEntityType=%d,   \n", entity.GetEntityType() );
                    
                    if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
                    {
                        mc_gState->m_TmpScript->SetElement(1);
                        err=mc_gState->m_TmpScript->GetNewEntityType(&new_entity_type,&entity_update,details_script,&details_script_size);
				if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: GetNewEntityType : new_entity_type=%d,  err=%d \n", new_entity_type, err);
                        if(err == 0)    // New entity element
                        {
                            if(entity_update == 0)
                            {
                    ...
                    {
                        if( entity.GetEntityType() != MC_ENT_TYPE_UPGRADE ) // (pseudo)stream
                        {
                            mc_gState->m_TmpScript->SetElement(1);
                                                                                // Should be spkk
            ...
            else                                                                // Not OP_RETURN
            {
                ...
                else
                {
                    do
                    {
                        ...
                        BOOST_FOREACH(const CTxDestination& addr, addressRets)
                        {
                            ...
                            {
                                //LogPrintf("[Full scanning] alreay existed Address = %s\n", CBitcoinAddress(addr).ToString().c_str());
                            }
                            //LogPrintf("[Full scanning] candiate size = %s\n", candidateAddress.size());
                        }
              ...
              //fNoDestinationInOutput >> there is destination
              // allow-arbitrary-outputs >> allowarbitraryoutputs >> MCP_ALLOW_ARBITRARY_OUTPUTS
              #if 0
              LogPrintf("[ASSET] :  :  : typeRet=%d, nRequiredRet=%d,   \n"
              , typeRet, nRequiredRet);
			  
	        LogPrintf("[ASSET] :  :  : fNoDestinationInOutput=%d, fShouldHaveDestination=%d, MCP_ALLOW_ARBITRARY_OUTPUTS=%d  \n"
              , fNoDestinationInOutput, fShouldHaveDestination, MCP_ALLOW_ARBITRARY_OUTPUTS);
		 #endif
                ...
                if (ithong) LogPrintf("[AcceptHdacTransaction] GetNumElements=%d \n", mc_gState->m_TmpScript->GetNumElements());
                ...
                for (int e = 0; e < mc_gState->m_TmpScript->GetNumElements(); e++)
                {
                    ...
                    {
                    //LogPrintf(" e=%d: %s, GetEntityType=%d \n", e, "Entity element", entity.GetEntityType());
                    ...
                    //LogPrintf(" :  :  :  e=%d: %s  \n", e, "Not Entity element");
                        ...
                        if(mc_gState->m_TmpScript->GetPermission(&type,&from,&to,&timestamp) == 0)// Permission script
                        {
                            ...
                            switch(pass)
                            {
                                case 0:                                         // Not admin or activate
                                    ...
                                    #ifdef FEATURE_HPAY_SUPPORT_BANNED_ADDRS
                                    type &= ( MC_PTP_CONNECT | MC_PTP_SEND | MC_PTP_RECEIVE | MC_PTP_WRITE | MC_PTP_BANNED);
                                    #else
                            ...
                            if(type)
                            {
                                ...
                                #ifdef FEATURE_HPAY_SKIP_VERIFY_PERM_WATCHONLY_ADDR
                                // watchonly address can't get mine permission
                                if(!fAdminFound)
                                {
                                    txnouttype tRet;
                                    int reqRet;
                                    vector<CTxDestination> addrRets;
                                    
                                    for(int i=0; i<tx.vout.size(); i++)
                                    {
                                        const CScript& scr = tx.vout[i].scriptPubKey; 
                                        if(ExtractDestinations(scr,tRet,addrRets,reqRet))
                                        {
                                            CBitcoinAddress addrs;
                                            CScriptID *scrID=boost::get<CScriptID> (&addrRets[0]);
                                            CKeyID *keyID=boost::get<CKeyID> (&addrRets[0]);
                                            if(keyID == NULL)
                                            {
                                                addrs=CBitcoinAddress(*scrID);
                                            }
                                            else
                                            {
                                                addrs=CBitcoinAddress(*keyID);
                                            }
                                            isminefilter mine = IsMine(*pwalletMain, addrs.Get());
                                ...
                                #endif/* FEATURE_HPAY_SKIP_VERIFY_PERM_WATCHONLY_ADDR */
                       ...
                       //LogPrintf(" :  :  :  e=%d: %s  \n", e, "Not permission script");
                            ...
                            #if 1//ithong_hmpl
                            if((pass==3) && (mc_gState->m_TmpScript->IsHmpl()))
                            {
                                hmplscript_head head;
                                unsigned char body[HDAC_HMPL_BUFF_MAX];
                                int hmpl_result = mc_gState->m_TmpScript->Gethmplinfo(&head, body);     
                                if(fDebug>1) LogPrint("hdactx"," :  :  :  GethmplRaw was here : %d  \n", hmpl_result);
                                if( hmpl_result== 0)// hmpl
                                {
                                    extern CCriticalSection cs_HMPLTransaction;	
                                    // process hmpl to here  
                                    LOCK(cs_HMPLTransaction);
                                    addHmplMsg(tx.GetHash(), &head, body);
                                     if(fDebug>1) LogPrint("hdactx"," :  :  :  GethmplRaw was here : mode=%d, svc_h=%d, svc_l=%d, size=%d \n", head.mode, head.svc_h, head.svc_l, head.size);
                                }
                            }
                            #endif
                        }
                ...
                if(pass == 3)
                {
                if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: 1673 : pass=%d,   \n", pass );
                    if(!mc_ExtractOutputAssetQuantities(reason))                // Filling output asset quantity list
                    {
                        fReject=true;
                        goto exitlbl;                                                                        
                    }
					if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: 1673 : \n" );
                }
                ...
                if( (pass == 3) && !fIsPurePermission )                         // Check for dust and receive permission
                {
                    ...
                    {
                    //LogPrintf(" :  :  :  %s  \n", "If not in block - part of IsStandard check");
                    ...
                    {                            
                        ...
                        if((lpKeyID == NULL) && (lpScriptID == NULL))
                        {
                        //LogPrintf(" :  :  :  %s  \n", "Script rejected - wrong destination type");
                ...
                do
                {
                    if(pass < 3 || fNoDestinationInOutput)
                        break;
                    ...
                    {
                        BOOST_FOREACH(CTxDestination& addr, addressRets)
                        {
                            CKeyID *lpKeyID=boost::get<CKeyID> (&addr);
                            CScriptID *lpScriptID=boost::get<CScriptID> (&addr);
                            if((lpKeyID != NULL) || (lpScriptID != NULL))
                            {
                                unsigned char* ptr=NULL;
                                if(lpKeyID != NULL)
                                ...
                                if(mc_gState->m_Permissions->IsBanned(NULL, ptr))
                                {
                                    char msg[100];
                                    sprintf(msg,"address [%s] is banned",CBitcoinAddress(addr).ToString().c_str());
                                    reason=msg;
                                    fReject=true;
                                    goto exitlbl;
                                }
	...
	//LogPrintf("[ASSET] AcceptHdacTransaction: 1801 :   \n" );
    ...
    if(!mc_CompareAssetQuantities(reason))                                      // Comparing input/output asset quantities
    {
        ...
        //LogPrintf("[ASSET] AcceptHdacTransaction: 1808 : \n" );
	...
	if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: 1808 :  nNewEntityOutput=%d, accept=%d, fFullReplayCheckRequired=%d \n", nNewEntityOutput , accept, fFullReplayCheckRequired);
    ...
    if(nNewEntityOutput >= 0)                                                   // (pseudo)streams and upgrade                                               
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            if( (vInputHashTypes[i] == SIGHASH_ALL) || ( (vInputHashTypes[i] == SIGHASH_SINGLE) && ((int)i == nNewEntityOutput) ) )
            {
                ...
                {                            
                    ...
                    {
                    if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: openers=%s :   \n" ,vInputDestinations[i].ToString());
		...
		if (ithong) LogPrintf("[ASSET] AcceptHdacTransaction: openers.size=%d : new_entity_type=%d, MaxStoredIssuers=%d, err=%d  \n" ,openers.size(), new_entity_type, mc_gState->m_Assets->MaxStoredIssuers(), err);
