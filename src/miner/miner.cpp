...

    {
        ...
        while (!vecPriority.empty())
        {
          ...
          */
            ...
            if (fPrintPriority)
            {
                if(fDebug>0)LogPrintf("Add tx to bloack : priority %.1f fee %s txid %s\n",
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("HdacMiner : mining is paused, generated block is dropped");	// HDAC
        }
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            return error("HdacMiner : generated block is stale");	// HDAC
        }
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HdacMiner : ProcessNewBlock, block not accepted");	// HDAC
