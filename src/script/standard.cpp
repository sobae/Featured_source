...
bool IsStandardNullData(const CScript& scriptPubKey,bool standard_check)
{
    ...
    //LogPrintf("IsStandardNullData : scriptPubKey.size() = %d, standard_check=%d \n",scriptPubKey.size(), standard_check);
    
    while( op_drop_count < max_op_drop_count+1 )
    {
        if(!scriptPubKey.GetOp(pc, opcode))
        {
           //LogPrintf("IsStandardNullData : return1 = %d \n",false);
            return false;
        }

		//LogPrintf("IsStandardNullData : opcode = %d \n",opcode);
        ...
        else
        {
            ...
            {
            //LogPrintf("IsStandardNullData : return2 = %d \n",false);
                return false;
            }
        }

		//LogPrintf("IsStandardNullData : op_drop_count = %d \n",op_drop_count);
        ...
        if(op_drop_count < max_op_drop_count+1)
        {
            ...
            else
            {
                ...
                else
                {
                    if(fixed)
                    {
                       //LogPrintf("IsStandardNullData : return3 = %d \n",false);
                        return false;
                    }
                }
            }
            ...
            if(!scriptPubKey.GetOp(pc, opcode))
            {
               //LogPrintf("IsStandardNullData : return4 = %d \n",false);
                return false;
            }
            ...
            if(opcode != OP_DROP)
            {
               //LogPrintf("IsStandardNullData : return5 = %d \n",false);
                return false;
            }
            op_drop_count++;
        }        
    ...
    //LogPrintf("IsStandardNullData : op_drop_count = %d, MAX_OP_RETURN_RELAY=%d, MAX_SCRIPT_ELEMENT_SIZE=%d \n",op_drop_count,MAX_OP_RETURN_RELAY,MAX_SCRIPT_ELEMENT_SIZE);
    ...
    if (pc < scriptPubKey.end())
    {
        if(pc + OP_PUSHDATA1 < scriptPubKey.end())
        {
            ...
            //LogPrintf("IsStandardNullData : 445 : vch.size() = %d \n",vch.size());
            if( !fixed || standard_check )
            {
                if (vch.size() > MAX_OP_RETURN_RELAY)
                {
                //LogPrintf("IsStandardNullData : return6 = %d \n",false);
                    return false;
                }
            }
        }
        else
        {
            ...
            if(opcode >= OP_PUSHDATA1)
            {
            //LogPrintf("IsStandardNullData : return7 = %d \n",false);
                return false;
            }
            if( !fixed || standard_check )
            {
                ...
                {
                //LogPrintf("IsStandardNullData : return8 = %d \n",false);
                    return false;
                }            
            }
        }
        if(scriptPubKey.GetOp(pc, opcode))
        {
        //LogPrintf("IsStandardNullData : return9 = %d \n",false);
            return false;
        }
    }

    //LogPrintf("IsStandardNullData : fixed = %d, recheck = %d \n",fixed,recheck);
    ...
    if( !fixed || standard_check )
    {
	    if(recheck)
	    {
	        ...
	        {
	            scriptPubKey.GetOp(pc, opcode, vch);
				//LogPrintf("IsStandardNullData : opcode = %d, op_drop_count=%d, vch.size=%d \n",opcode,op_drop_count,vch.size());
	            if(opcode == OP_RETURN)
	            {
	                op_drop_count=max_op_drop_count+1;
	            }
	    ...
	    {
	    //LogPrintf("IsStandardNullData : op_drop_count=%d, sizes = %d \n",op_drop_count,sizes[op_drop_count]);
	        if( sizes[op_drop_count] > MAX_SCRIPT_ELEMENT_SIZE )
	        {
	        //LogPrintf("IsStandardNullData : return10 = %d \n",false);
	            return false;
	        }            
	    }
    }

	//LogPrintf("IsStandardNullData : return11 = %d \n",true);
...
bool IsStandard(const CScript& scriptPubKey, txnouttype& whichType)
{
	...
	//LogPrintf("IsStandard : scriptPubKey.size() = %d, \n",scriptPubKey.size());
	
    if (scriptPubKey.GetOp(pc, opcode))
    {
   // LogPrintf("IsStandard : opcode = %d, \n",opcode);
        if(opcode == OP_DUP)
        {
            if ( !scriptPubKey.GetOp(pc, opcode) || (opcode != OP_HASH160) )
            {
