...
#if 0//ithong_free_fee
//bool IsStandardTx(const CTransaction& tx, string& reason, bool check_for_dust, unsigned int *op_ret_cnt)
bool IsFreeNullData(const CTransaction& tx, CAmount nValueIn, unsigned int tx_size)
{
    AssertLockHeld(cs_main);
...
CAmount GetMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    if(nMinFee>maxTxFee)
        nMinFee = maxTxFee;
    #else /* FEATURE_HPAY_FEE_FIX */
      #error should be deifined feature....
...
bool CheckFreeHMPL(CTransaction txNew, const CCoinsViewCache& mapInputs)
{
    ...
    CAmount nTotalOutValue = mapInputs.GetValueIn(txNew);
    ...
    //LogPrintf(" CheckHMPL_AMP: vin.size=%d, vout.size=%d, nInValue=%d \n", vin_size, vout_size, nTotalOutValue);    
    ...
    for (m = 0; m < txNew.vout.size();m++)
    {
        ...
        if(ExtractDestinationScriptValid(script_temp, addressRet))
        {
            ...
            if(lpKeyID != NULL)
            {
                ...
                {  
                    hmpl_perm_t |= mc_gState->m_Permissions->CanAdmin(NULL,(unsigned char*)(lpKeyID));
                    LogPrintf(" CheckHMPL: hmpl_perm_t=%08x, nValueOut=%d \n", hmpl_perm_t, txNew.vout[m].nValue);  
    ...
    LogPrintf(" CheckHMPL: hmpl_included=%d, hmpl_perm_t=%08x, hmpl_outValue=%d, hmpl_type=%d \n"
		,hmpl_included,hmpl_perm|hmpl_perm_t,hmpl_outValue,hmpl_mode);  
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee,bool fAddToWallet)
{
...

    {
       ...
       #ifdef FEATURE_HPAY_FEE_FIX
        //LogPrintf("[acceptMP] Try with : hash=%s, tx nSize=%d, nFees=%d, minRelayTxFee=%d \n"			                  , hash.ToString(), nSize, nFees, ::minRelayTxFee.GetFee(nSize));        


        #if 1//ithong_hmpl
        if((nSize<1000) && CheckFreeHMPL(tx, view))
        {
            if(fDebug>1) LogPrint("main"," CheckHMPL: allow free relay fee for admin service \n");  
        }
        else
        {
        ...
        if (fLimitFree && nFees < ::minRelayTxFee.GetFee(nSize))
        {
            ...
            if(fDebug>1)LogPrint("mempool", "Rate limit dFreeCount: %g => %g\n", dFreeCount, dFreeCount+nSize);
            dFreeCount += nSize;
        }
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if(chainActive.Tip()->kMiner.IsValid())
    {
        ...
        if(fDebug>0)LogPrintf(",A,UpdateTip,%d,%s\n", chainActive.Tip()->nHeight,addr.ToString().c_str());//ithong
    }
...
bool ProcessNewBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
        ...
        
    {
		...
		{
		      std::string addrMiner2 = GetCoinbaseAddress(*pblock);
			...
			std::string msg = strprintf("%d,CMINER,%s,from,%s,WZ,%d,NF,%d", bh+1, addrMiner2,(pfrom == NULL ? "ME": pfrom->addr.ToString()), wz, nf);
			//if(fDebug>0)LogPrintf("hdac: %s\n", msg);
			if(fDebug>0)LogPrintf(",AB,ProcessNewBlock,%s\n", msg);//ithong
		}
		/* HDAC END */
...
/*
    ...
    {
        if(fDebug>1)LogPrint("mchn","mchn: New block rejected as chain tip\n");
        if(pblockindex->nHeightMinedByMe == pblockindex->nHeight)
        {
