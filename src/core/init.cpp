...
#ifdef FEATURE_HPAY_TX_CONFIRM_TARGET
#define DEFAULT_TX_CONFIRM_TARGET    6
#endif/* FEATURE_HPAY_TX_CONFIRM_TARGET */
...
#ifdef FEATURE_HPAY_LIMITFREERELAY_NOT_USED
  // not used..
#else
...
#endif/* FEATURE_HPAY_LIMITFREERELAY_NOT_USED */
...
bool AppInit2(boost::thread_group& threadGroup,int OutputPipe)
{
    ...
    #endif/* FEATURE_HPAY_UPDATE_PARAMS_HASH */
    //LogPrintf("[AppInit2] Step  Step 1: setup..\n");
    // ********************************************************* Step 1: setup
#ifdef _MSC_VER
...
#endif
//LogPrintf("[AppInit2] Step  2: parameter interactions..\n");
...
//LogPrintf("[AppInit2] Step  Step3: parameter-to-internal-flags..\n");
	...
	//LogPrintf("[AppInit2] Step  Step 4: application initialization: dir lock, daemonize, pidfile, debug log...\n");
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    */
    int64_t nStart;
//LogPrintf("[AppInit2] Step 5: verify wallet database integrity...\n");
           ...
           || (mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL))
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to Hdac 1.0 beta 2 or later"));	// HDAC
                        }
            ...
            */
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_node))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart hdacd";	// HDAC
                }
                else
                {
                    if(seed_port == 0)
            ...
            {                                
                if( (mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL) &&
                    ...
                    (strcmp((char*)mc_gState->m_NetworkParams->GetParam("chainprotocol",NULL),"hdac") == 0) )	// HDAC
                {
                ...
                if(pwalletMain->vchDefaultKey.IsValid())
                {
                    ...
                    {    
                        ...
                        sprintf(bufOutput,"hdac-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                             CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());	// HDAC
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
...
//LogPrintf("[AppInit2] Step 6: network initialization...\n");
    //RegisterNodeSignals(GetNodeSignals());
    ...
    if(!GetBoolArg("-shortoutput", false))
    {
        if(fListen && !GetBoolArg("-offline",false))
        {
            ...
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            sprintf(bufOutput,"hdacd %s:%d\n\n",HdacServerAddress().c_str(),GetListenPort());	// HDAC
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            if(found_ips > 1)
            {
                ...
                for(int i_ips=0;i_ips<found_ips;i_ips++)
                {
                    if(all_ips[i_ips] != mc_gState->m_IPv4Address)
                    {
                        ...
                        sprintf(bufOutput,"hdacd %s@%u.%u.%u.%u:%d\n",mc_gState->m_NetworkParams->Name(),ptr[3],ptr[2],ptr[1],ptr[0],GetListenPort());	// HDAC
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            ...
            {            
                ...
                BOOST_FOREACH(string strAddr, mapMultiArgs["-externalip"]) 
                {
                    ...
                    if(port>0)
                    {
                        sprintf(bufOutput,"hdacd %s@%s\n",mc_gState->m_NetworkParams->Name(),strAddr.c_str());	// HDAC
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                        port=GetListenPort();
                    }
                    else
                    {
                        sprintf(bufOutput,"hdacd %s@%s:%d\n",mc_gState->m_NetworkParams->Name(),strAddr.c_str(),GetListenPort());	// HDAC
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));                    
                    }
                }
                sprintf(bufOutput,"\n");
...
//LogPrintf("[AppInit2] Step 7: load block chain...\n");
    ...
    //LogPrintf("[AppInit2] Step 8: load wallet...\n");
    // ********************************************************* Step 8: load wallet
#ifdef ENABLE_WALLET
    if (fDisableWallet) {
        pwalletMain = NULL;
...
//LogPrintf("[AppInit2] Step Step 9: import blocks..\n");
...
//LogPrintf("[AppInit2] Step 10: start node..\n");
    // ********************************************************* Step 10: start node
    ...
    if(!GetBoolArg("-offline",false))
    {
        ...
        if (pwalletMain)
        	GenerateBitcoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", 1));	// HDAC
#endif

#if 1//ithong_hmpl
        if(GetBoolArg("-hmpl",false))
        {
            LogPrintf("AppInit2 : start ProcHMPL.\n");
            ProcHMPL(true);
        }
#endif

//LogPrintf("[AppInit2] Step 11: finished..\n");
    // ********************************************************* Step 11: finished
    /*
