...
#ifdef FEATURE_HPAY_GET_PEER_NODE_BLOCK_HEIGHT
...
#endif/* FEATURE_HPAY_GET_PEER_NODE_BLOCK_HEIGHT */
...
Value getnetstat(const Array& params, bool fHelp)
{
    ...
    Object obj;
    uint64_t time_S = GetNetStartedTime();
    uint64_t time_E = GetTimeMillis();
    uint64_t relayTx = GetTransactionCountToRelay();
    uint64_t relayed = GetTransactionCountRelayed();

    if(params.size()==1) 
    { 
        string s_overall;
        s_overall += strprintf("Start,%u,", time_S);
        s_overall += strprintf("Current,%u,", time_E);
        s_overall += strprintf("TotalTxToRelay,%u,", relayTx);
        s_overall += strprintf("TotalTxRelayed,%u,", relayed);
        obj.push_back(Pair("overall", s_overall));   
    }
    else
    {
        obj.push_back(Pair("Start Time", time_S));
        obj.push_back(Pair("Current Time", time_E));
        obj.push_back(Pair("TotalTxToRelay", relayTx));
        obj.push_back(Pair("TotalTxRelayed", relayed));
    }
	
    return obj;
}

Value resetnetstat(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error("Help message not found\n");
