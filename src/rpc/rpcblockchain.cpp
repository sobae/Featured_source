...
#if 1//ithong_stat
Value getblocklocal(const Array& params, bool fHelp)
{
    ...
    int stopHeight = 0;
    int nHeight = 0;
    int nLimit = (int)chainActive.Height();

    startHeight = atoi(params[0].get_str().c_str());
    stopHeight = atoi(params[1].get_str().c_str());

    LogPrintf("getblocklocal : %d ~ %d\n", startHeight, stopHeight);

    if((startHeight<0) || (startHeight>stopHeight)|| (stopHeight>nLimit))
        throw JSONRPCError(RPC_BLOCK_NOT_FOUND, "Abnormal arguements !!!");

    Array ret;

    Object o;
    std::string msg_ref = strprintf("%s,index,nTime,nBits,nNonce,nChainWork,maddr,caddr","ref");
    o.push_back(Pair("ref", msg_ref));
    ret.push_back(o);

    for(int m=startHeight;m<stopHeight;m++)
    {
        std::string strHash = chainActive[m]->GetBlockHash().GetHex();  
        uint256 hash(strHash);
        Object o;
        ...
        std::string caddr = GetCoinbaseAddress(block);

#if 0
        if(m==startHeight)
        {
            Object o1;
            std::string msg_ref = strprintf("%s,index,nTime,nBits,nNonce,maddr,caddr","ref");
            o1.push_back(Pair("ref", msg_ref));            
            ret.push_back(o1);
        }
#endif

        std::string msg = strprintf(":msg,%d,%u,%08x,%u,%u,%s,%s", m, block.nTime, block.nBits, block.nNonce, (uint64_t)(pblockindex->nChainWork.getdouble()/1000000000),maddr.ToString().c_str(), caddr);
        o.push_back(Pair("msg1", msg));
