...
#if 1//ithong_hmpl
//pushhmpl <type> <svc_h> <svc_l> <from> <to> <metadata> <amount> 
Value pushhmpl(const Array& params, bool fHelp)
{
    vector<CTxDestination> fromaddresses, toaddresses;
    uint16_t mode = 0;
    uint32_t svc_h = 0, svc_l = 0;
    CAmount nAmount = 0;

    mode = (uint16_t)atoi64(params[0].get_str());
    svc_h = (uint32_t)atoi64(params[1].get_str());
    svc_l = (uint32_t)atoi64(params[2].get_str());
	
    fromaddresses=ParseAddresses(params[3].get_str(),false,false);
    toaddresses=ParseAddresses(params[4].get_str(),false,false);

	//return 0;
    nAmount = AmountFromValue(atoi64(params[6].get_str()));
    //LogPrintf("pushhmpl :  nAmount=%d...\n", nAmount);
	//LogPrintf("pushhmpl :  nAmount=%f...\n", nAmount);
    ...
    {
        bool fIsHex;
        vector<unsigned char> dataData(ParseHex(params[5].get_str().c_str(),fIsHex));    
        if(!fIsHex)
        {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "data-hex should be hexadecimal string or recognized object format");                        
        }
		//LogPrintf(" pushhmpl: type=%d, dataData.size=%d, nAmount=%d \n", mode, dataData.size(), nAmount);
		
        lpScript->SetHmplRaw(mode, svc_h, svc_l, (uint16_t)dataData.size(), (unsigned char*)&dataData[0]);
    }
...
//return 0;
    ...
    SendMoneyToSeveralAddresses(toaddresses, nAmount, wtx, lpScript, scriptOpReturn, fromaddresses);
    
    return wtx.GetHash().GetHex();
}

//listhmpl 
Value listhmpl(const Array& params, bool fHelp)
{
    Array ret;    
	
    std::map<uint256, HmplMsgType> mapHmpl;

    GetHmplList(mapHmpl);

    for (std::map<uint256, HmplMsgType> ::const_iterator mi = mapHmpl.begin();mi != mapHmpl.end();++mi)
    {
        Object o;
        o.push_back(Pair("tx_hash", mi->first.ToString()));
        ret.push_back(o);
    }
