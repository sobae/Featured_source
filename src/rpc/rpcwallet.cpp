...
CAmount GetAccountBalance(const string& strAccount, int nMinDepth, const isminefilter& filter)
{
 ...
 //LogPrintf(" %s : %d : GetAccountBalance :  strAccount=%s, nMinDepth=%d, filter=%d \n", __FILE__, __LINE__, strAccount, nMinDepth, filter);
        ...
        {           
            ...
            if(CBitcoinAddressFromTxEntity(addressEntity,&(lpEntity->m_Entity)))
            {
                BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
                {
                    ...
                    if(strAccount.size())
                    {
                        //LogPrintf(" %s : %d : GetAccountBalance :  addressEntity=%s,  \n", __FILE__, __LINE__, addressEntity.ToString());
	...
	//LogPrintf(" %s : %d : GetAccountBalance :  setAddress=%d,  \n", __FILE__, __LINE__, setAddress.size());
    ...
    //LogPrintf("[ITHONG] totalBTC=%d with %d addresses for accounts=%s \n",totalBTC, setAddress.size(), strAccount);
...
Value settxfee(const Array& params, bool fHelp)
{
    ...
    //LogPrintf("[RPCWALLET] settxfee : %s \n", payTxFee.ToString());
