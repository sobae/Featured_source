...
bool VerifyBlockWindow(const CBlock& block, CNode* pfrom)
{
	...
	diffBlkTime = now - blockTime;
       ...
       //LogPrintf("[ITHONG] VerifyBlockWindow : DIFF(now vs block): %d , ref : %d(sec), for big then continue  \n", diffBlkTime,  MCP_TARGET_BLOCK_TIME*HDAC_TARGET_BLOCK_TIME_MULTIPLE);
	...
	if(IsMiningBlackout())	// CIH ??????? - ponder more deeply
	{
		...
		//LogPrintf("[ITHONG] VerifyBlockWindow : miningContinuity %d , nf: %d, fRet=%d \n", miningContinuity,  nf, fRet);
...
bool IsMiningBlackout()
{
	...
	//LogPrintf("[ITHONG] IsMiningBlackout : DIFF(now vs tip): %d, ref:%d(sec) \n", diffBlkTime,  MCP_TARGET_BLOCK_TIME*HDAC_TARGET_BLOCK_TIME_MULTIPLE);
...
//LogPrintf("[ITHONG] CheckBlockWindow : miningDepth %d , wz: %d,  \n", miningDepth,  wz);
...
//LogPrintf("[ITHONG] CheckePoWRule : miningContinuity %d , wz: %d,  nf=%d, blockHeight=%d \n", miningContinuity,  wz, nf, blockHeight);
